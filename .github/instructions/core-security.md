...
| Data Type | Retention Period | Justification | Cleanup Schedule |
|-----------|-----------------|---------------|-----------------|
| **Chat messages** | 90 days | Business requirement | Daily cron job |
| **Payment records** | 7 years | Canadian Tax Law (CRA requirement) | Manual (never auto-delete) |
| **User accounts** | Until deleted by user | User choice | On user request |
| **Audit logs** | 1 year minimum | PIPEDA requirement (security & compliance) | Weekly cron job (keep 1 year) |
| **Session data** | 7 days (refresh token) | Security | Daily cron job |
| **Notification history** | 90 days (unread kept) | Business requirement | Daily cron job |
...
@Throttle({ default: { limit: 3, ttl: 3600000 } }) // 3 requests per hour (stricter)
@Post('register')
async register() { /* ... */ }
